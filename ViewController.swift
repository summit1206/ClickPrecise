...
class ViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    if UserDefaults.standard.value(forKey: "highscore_normal") != nil {
      ...
      highScoreLabel.text = "Your record: \(self.highscoreFromUserDefaults)"
    } else {
      highScoreLabel.text = "Your record: 0"
    }
    ...
    self.navigationItem.title = "Click Precise"
    self.helloButtonWithPlayerName.setTitle("Hey, \(self.nameFromUserDefaults)", for: .normal)
  ...
  @IBAction func switchModeDidTapped(_ sender: Any) {
    if switchModeGame.isOn == false {
      self.faultLabel.text = "Accuracy: from 0.05 ms"
      isHarcoreMode = false
      scoreRef = rootRef.child("leaderboards_normal").child(nameFromUserDefaults)
    } else {
      self.faultLabel.text = "Accuracy is disabled"
      isHarcoreMode = true
  ...
  @IBAction func logOutButtonDidTapped(_ sender: Any) {
    ...
    let exitAction = UIAlertAction(title: "Sign out", style: .destructive, handler: { _ in self.exitClicked() })
  ...
  func gameOver() {
    ...
    if count > highscoreFromUserDefaults {
      ...
      highScoreLabel.text = "Your record: \(highscoreFromUserDefaults)"
      ...
      appDelegate.simpleMsg(title: "Congratulations!", text: "You broke the record. Your new score is \(count) points", colorBg: alertBackgroundColor, colorText: .white, iconText: "🎉")
    }
  ...
  @IBAction func shareButtonDidTapped(_ sender: Any) {
    let activityVC = UIActivityViewController(activityItems: ["Hey, my record in HardcoreTap: \(self.highscoreFromUserDefaults). Try to collect more;) appstore.com/hardcoretap"], applicationActivities: nil)
